...
const init = async () => {
    ...
    server.route({
        ...
        handler: async(req, res) =>{
            try {
                const data = await knex('hapi_js').where({id: req.params.id})
    ...
    server.route({
        ...
        handler: async (req, res) => {
            ...
            try {
                console.log(req.payload);
                const data = await knex('hapi_js').where({email: req.payload.email}).update(req.payload)
    ...
    server.route({
        ...
        handler: async (req, h) => {
            ...
            try {
                const data = await knex('hapi.js').where({email: req.payload.email}).del(req.payload)
                return 'data deleted'
            } catch (err) {
                return err;
